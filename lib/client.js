...
Client.prototype.on_swank_message = function(msg) {
  ...
  if (cmd == ":return") {
    ...
    this.on_handlers.print_string(sexp.children[1].source.slice(1,-1));
  } else if (cmd == ':presentation-start') {
    this.on_handlers.presentation_start();
  } else if (cmd == ':presentation-end') {
    this.on_handlers.presentation_end();
...
Client.prototype.compile_string = function(compile_string, filename, filename_full, position, line, column, package) {
  var cmd = "(SWANK:COMPILE-STRING-FOR-EMACS \"" +  compile_string.replace(/\"/g, '\\"') + "\" \"" + filename + "\" '((:POSITION " + position + ") (:LINE " + line + " " + column + ")) \"" + filename_full + "\" 'NIL)";
  compile_string.replace(/\"/g, '\\"')
  console.log(cmd);

...
Client.prototype.socket_data_handler = function(data) {
  var source = new Buffer.from(data);
  var byte_offset = 0; // offset into source that has been copied so far

  while (byte_offset < source.byteLength) {
    var amount_to_read = Math.min(this.len_remaining, source.byteLength - byte_offset);
    var copied = source.copy(this.buffer, this.buffer.byteLength - this.len_remaining, byte_offset, byte_offset + amount_to_read);
...
Client.prototype.setup_read = function(length, fn) {
  this.buffer = new Buffer.alloc(length);
